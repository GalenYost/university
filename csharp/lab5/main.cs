...
public class TestCollections<TKey, TValue> where TKey : class where TValue : TKey {
   ...
   public string MeasureSearchTimes () {
      if (persons.Count == 0) return "Collections are empty.";
      ...
      sw.Restart(); found = persons.Contains(targetPerson); sw.Stop();
      results.Add($"List<Person>.Contains: {sw.ElapsedTicks} ticks | found={found}");

      sw.Restart(); found = strings.Contains(targetString); sw.Stop();
      results.Add($"List<string>.Contains: {sw.ElapsedTicks} ticks | found={found}");

      sw.Restart(); found = personToMusician.ContainsKey(targetPerson); sw.Stop();
      results.Add($"Dictionary<Person,Musician>.ContainsKey: {sw.ElapsedTicks} ticks | found={found}");

      sw.Restart(); found = personToMusician.ContainsValue(targetMusician); sw.Stop();
      results.Add($"Dictionary<Person,Musician>.ContainsValue: {sw.ElapsedTicks} ticks | found={found}");

      sw.Restart(); found = stringToMusician.ContainsKey(targetString); sw.Stop();
      results.Add($"Dictionary<string,Musician>.ContainsKey: {sw.ElapsedTicks} ticks | found={found}");

      sw.Restart(); found = stringToMusician.ContainsValue(targetMusician); sw.Stop();
      results.Add($"Dictionary<string,Musician>.ContainsValue: {sw.ElapsedTicks} ticks | found={found}");
...
public class Musician : Person, IDateAndCopy, IEnumerable {
   ...
   public double Rating {
      get => rating;
      set {
         if (value <= 0 || value > 5) throw new ArgumentOutOfRangeException(nameof(value), "Rating must be between 1 and 5");
         rating = value;
      }
   }
...
public class MusicianCollection {
   ...
   public override string ToString () {
      ...
      foreach (var m in musicians) {
         ...
         sb.AppendLine("  Songs:");
         foreach (var song in m.Songs) sb.AppendLine($"    - {song}");

         sb.AppendLine("  Concerts:");
         foreach (var c in m.Concerts) sb.AppendLine($"    - {c:d}");
         sb.AppendLine();
      }
      return sb.ToString();
   }
   ...
   public virtual string ToShortString () {
      ...
      foreach (var m in musicians) {
        ...
        sb.Append($"last song: {lastSong}, total songs: {songCount}, total concerts: {concertCount}");
        sb.AppendLine();
      }
      return sb.ToString();
   }
...
public class Program {
   public static void Main () {
      ...
      collection.AddMusicians(
         ...
         new Musician { Name = "Zoya", Surname = "Abramova", Birthday = new DateTime(1995, 12, 5), Rating = 4.1 }
      );
